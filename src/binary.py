...
class BinaryInput:
    ...
    def get_this_trick_lead_suit(self):
        ...
        for k in (0, 1, 2):
            ...
            this_trick_lead_suit[
                was_played & (~lead_found), card_suit[was_played & (~lead_found)]
            ] = 1
...
CARD_INDEX_LOOKUP = dict(
    zip(["A", "K", "Q", "J", "T", "9", "8", "7", "6", "5", "4", "3", "2"], range(13))
)
...
def get_auction_binary_sampling(
    n_steps, auction_input, hand_ix, hand, vuln, ns=0, ew=0
):
    ...
    vuln_us_them = np.array(
        [vuln[hand_ix % 2], vuln[(hand_ix + 1) % 2]], dtype=np.float32
    )
    ...
    auction_input = (
        auction_input + ["PAD_END"] * 4 * n_steps
    )  # To prevent index errors in the next section
    auction = bidding.BID2ID["PAD_END"] * np.ones(
        (n_samples, len(auction_input)), dtype=np.int32
    )
    ...
    if hand_ix % 2 == 0:
        ...
        X_padded = np.pad(
            X_padded, padding_width, mode="constant", constant_values=(ns)
        )
    else:
        X_padded = np.pad(X, padding_width, mode="constant", constant_values=(ns))
        X_padded = np.pad(
            X_padded, padding_width, mode="constant", constant_values=(ew)
        )
...
def get_shape_for_lead(auction, hand, binfo, vuln, ns=0, ew=0):
    b = np.zeros(15)
...
def get_auction_binary_for_lead(auction, hand, binfo, vuln, ns=0, ew=0):
    contract = bidding.get_contract(auction)
    if contract is None:
