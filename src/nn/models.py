    ...
    def from_conf(cls, conf: ConfigParser) -> "Models":
        return cls(
            bidder_model=Bidder('bidder', os.path.join(
                BIDDER_MODEL_BASE_PATH, conf['bidding']['bidder'])),
            wide_bidder_model=Bidder('bidder', os.path.join(
                BIDDER_MODEL_BASE_PATH, conf['bidding']['wide_bidder'])),
            binfo=BidInfo(os.path.join(
                BIDDER_MODEL_BASE_PATH, conf['bidding']['info'])),
            lead=Leader(os.path.join(
                BIDDER_MODEL_BASE_PATH, conf['lead']['lead'])),
            sd_model=LeadSingleDummy(os.path.join(
                BIDDER_MODEL_BASE_PATH, conf['eval']['lead_single_dummy'])),
            player_models=[
                player.BatchPlayerLefty('lefty', os.path.join(
                    BIDDER_MODEL_BASE_PATH, conf['cardplay']['lefty'])),
                player.BatchPlayer('dummy', os.path.join(
                    BIDDER_MODEL_BASE_PATH, conf['cardplay']['dummy'])),
                player.BatchPlayer('righty', os.path.join(
                    BIDDER_MODEL_BASE_PATH, conf['cardplay']['righty'])),
                player.BatchPlayer('decl', os.path.join(
                    BIDDER_MODEL_BASE_PATH, conf['cardplay']['decl']))
            ],
        )
