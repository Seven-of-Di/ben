...
def get_trick_winner_i(trick, strain):
    ...
    is_trumped = np.any(trick_cards_suit == trump_suit_i,
                        axis=1).reshape((-1, 1))

    trick_cards_trumps = trick_cards.astype(float)
    trick_cards_trumps[trick_cards_suit != trump_suit_i] = 99
    # adding random to break ties
    trick_cards_trumps += np.random.randn(n_samples, 4) / 1000
    ...
    # adding random to break ties
    trick_cards_lead_suit += np.random.randn(n_samples, 4) / 1000
    highest_lead_suit_i = np.argmin(
        trick_cards_lead_suit, axis=1).reshape((-1, 1))
