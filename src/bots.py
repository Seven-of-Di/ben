...
class BotBid:
    ...
    def bid(self, auction):
        ...
        if BotBid.do_rollout(auction, candidates):
            ...
            for candidate in candidates:
                auctions_np = self.bidding_rollout(
                    auction, candidate.bid, hands_np)
    ...
    def bidding_rollout(self, auction_so_far, candidate_bid, hands_np):
        ...
        for i in range(1, 5):
            n_steps_vals[(len(auction_so_far) % 4 + i) %
                         4] = BotBid.get_n_steps_auction(auction_so_far + ['?'] * i)

        # initialize auction vector
        auction_np = np.ones((n_samples, 64), dtype=np.int32) * \
        ...
        while not np.all(auction_np[:, bid_i] == bidding.BID2ID['PAD_END']):
            X = binary.get_auction_binary(
                n_steps_vals[turn_i], auction_np, turn_i, hands_np[:, turn_i, :], self.vuln)
            bid_np = self.model.model_seq(X).reshape(
                (n_samples, n_steps_vals[turn_i], -1))[:, -1, :]
    ...
    def expected_tricks(self, hands_np, auctions_np):
        ...
        for i in range(n_samples):
            sample_auction = [bidding.ID2BID[bid_i]
                              for bid_i in list(auctions_np[i, :]) if bid_i != 1]
        ...
        # lefty
        X_sd[:, (32 + 5 + 0*32):(32 + 5 + 1*32)
             ] = hands_np[s_all, (declarers + 1) % 4]
        # dummy
        X_sd[:, (32 + 5 + 1*32):(32 + 5 + 2*32)
             ] = hands_np[s_all, (declarers + 2) % 4]
        # righty
        X_sd[:, (32 + 5 + 2*32):(32 + 5 + 3*32)
             ] = hands_np[s_all, (declarers + 3) % 4]
        # declarer
...
class BotLead:
    ...
    def simulate_outcomes(self, n_samples, auction, lead_card_indexes):
        ...
        accepted_samples = sample.sample_cards_auction(
            n_samples, n_steps, auction, lead_index, self.hand, self.vuln, self.bidder_model, self.binfo_model)
        ...
        X_sd[:, (32 + 5 + 1*32):(32 + 5 + 2*32)] = accepted_samples[:,
                                                                    0, :].reshape((n_accepted, 32))
        # righty
        X_sd[:, (32 + 5 + 2*32):(32 + 5 + 3*32)] = accepted_samples[:,
                                                                    1, :].reshape((n_accepted, 32))
        # declarer
...
class CardPlayer:
    ...
    def set_card_played(self, trick_i, leader_i, i, card):
        played_to_the_trick_already = (
            i - leader_i) % 4 > (self.player_i - leader_i) % 4
    ...
    def next_card(self, trick_i, leader_i, current_trick, players_states, card_dd) -> str:
        ...
        max_expected_score = max([float(c.expected_score)
                                 for c in candidate_cards if c.expected_score is not None])
        card_with_max_expected_score = {
            c: c.expected_score for c in candidate_cards if c.expected_score == max_expected_score}
